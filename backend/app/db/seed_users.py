from __future__ import annotations

import os

import psycopg2

from app.auth.security import hash_password
from app.db.config import DATABASE_URL
from app.runtime_security import validate_runtime_security

SEED_ACCOUNTS = [
    ("user", os.environ.get("SEED_USER_PASSWORD", "user"), "user", None),
    ("admin", os.environ.get("SEED_ADMIN_PASSWORD", "admin"), "admin", None),
    ("c001", os.environ.get("SEED_C001_PASSWORD", "c001"), "customer", "C001"),
    ("b001", os.environ.get("SEED_B001_PASSWORD", "b001"), "customer", "B001"),
    ("c019", os.environ.get("SEED_C019_PASSWORD", "c019"), "customer", "C019"),
]


def seed_users(database_url: str = DATABASE_URL) -> dict[str, str]:

    validate_runtime_security()
    conn = psycopg2.connect(database_url)
    out: dict[str, str] = {}
    try:
        with conn.cursor() as cur:
            for username, password, role, owner_id in SEED_ACCOUNTS:
                cur.execute(
                    "INSERT INTO users (username, pass_hash, role, owner_id) VALUES (%s, %s, %s, %s) "
                    "ON CONFLICT (username) DO NOTHING",
                    (username, hash_password(password), role, owner_id),
                )
                out[username] = role
        conn.commit()
    except Exception:
        conn.rollback()
        raise
    finally:
        conn.close()
    return out


if __name__ == "__main__":
    result = seed_users()
    for username, role in result.items():
        print(f"user seeded: {username} ({role})")
