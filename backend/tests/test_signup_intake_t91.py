...
def test_present_form_card_shape():
    ...
    assert FORM_REQUIRED == [f["name"] for f in FORM_FIELDS]
...
def _register_and_conv() -> tuple[str, dict, str]:
    ...
    ck = r.cookies
    conv = client.post("/api/conversations", json={"title": "loan"}, cookies=ck).json()["id"]
    return u, ck, conv
...
def test_form_submit_happy_creates_c9xx_and_links():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cust = cur.fetchone()
            assert cust[0] == "Test Customer" and cust[1] == 25000000
...
def test_form_submit_bad_income_400():
    ...
    try:
        card_id = _mk_form_card(conv)
        bad = {**_GOOD_VALUES, "monthly_income": "not a number"}
        r = _submit(conv, card_id, ck, bad)
        assert r.status_code == 400
...
def test_new_customer_classify_yellow_honest_null():
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO customers (id, full_name, monthly_income) VALUES ('C990','New Customer T91',20000000) "
    ...
    try:
        ...
        assert r["item"]["lane"] == "yellow", (
            f"new customer without records must be honest-null yellow: {r['item']['lane']}"
        )
    finally:
        a.close_cursors()
        release(pg)
