...
def test_row_index_access():
    r = Row(("Test Customer", 30000000), ["full_name", "monthly_income"])
    assert r[0] == "Test Customer"
    assert r[1] == 30000000


def test_row_mapping_access():
    r = Row(("Test Customer", 30000000), ["full_name", "monthly_income"])
    assert r["full_name"] == "Test Customer"
...
def test_rewrite_question_mark_in_literal_preserved():
    ...
    sql = "SELECT * FROM t WHERE note='is this true?' AND id=?"
    got = _rewrite_placeholders(sql)
    assert "'is this true?'" in got
